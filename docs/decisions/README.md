...
|---|---|---|
| ADR-001 | 复用 `tasks` 表，`platform='atask'` | 复用 `tasks` 表，`platform='stask'`（约束细节不同） |
...
| 编号 | 标题 | Status | 一句话结论 |
|---|---|---|---|
| [ADR-001](ADR-001-reuse-newapi-tasks-table.md) | 复用 new-api `tasks` 表，网关零建表 | Accepted (2026-09-12) | 任务事实源 = new-api `tasks` 表，`platform='atask'` 划分；网关零自有表，自有状态全在 Redis |
...
| [ADR-008](ADR-008-division-with-stask-service.md) | 与 stask-service 的分工——不合并 | Accepted (2026-09-12) | 两者**都是任务队列服务**（不要把两者合并）；分工按**当前准入哪种任务**划分——atask 只接受**异步任务**（做**排队异步**：上游异步 → 本地异步）、stask 只接受**同步任务**（由它任务化），对外前缀**共用 `/async`**（由 nginx 按路径分流并重写，atask 内部端点是 `/queue`）；共用 `tasks` 表但 `platform` 不同 |
| [ADR-009](ADR-009-exception-layering-and-module-locality.md) | 异常分层——不引入统一基类 | Accepted (2026-09-12) | 异常按归属分置（端口契约异常跟端口模块放、模块私有异常就近定义；ADR-010 后端口层已删，该分类当前为空）；**刻意不建统一基类**——异常处置是逐点决策（现行见 ADR-010 三档分流），继承层级帮不上忙 |
| [ADR-010](ADR-010-queue-path-zero-billing.md) | 对外形态统一 `/queue/{上游路径}`，鉴权与计费全部下沉上游 | Accepted (2026-09-12) | 唯一形态 `POST/GET/DELETE /queue/{path}`；用户 token 原样透传、网关零资金动作、渠道路由零配置；**supersedes ADR-002/005/006/007、rewrites ADR-008** |
| [ADR-011](ADR-011-batch-release-gating.md) | 攒批放行——把「上游提交」从受理时刻解耦 | Accepted (2026-09-13) | 攒够 N 条或等够 T 秒才整批**提交上游**（不合并请求）；等待期**不占并发槽**（故受理不再 429，改为排队）；两层幂等（批次级 Redis 摘取 + 成员级 DB 条件更新）缺一不可；**只辖「上游提交时机」，是 ADR-010 的增量** |
| [ADR-012](ADR-012-callback-address-admission.md) | 用户回调地址的准入与投递 | Accepted (2026-09-13) | 取值「`X-Callback-Url` 头优先、body 顶层 `callback_url` 兜底」；地址过 fail-closed 白名单（`CALLBACK_ALLOWLIST` 空即全拒、私网字面 IP 无条件拒）；网关接管模式下把该字段从转发体摘除（防双投递），透传模式则取值与校验一起跳过；**只推终态**（中继观测到的中间态是抽样而非事件流） |
...
1. **ADR-010** —— 现行架构的唯一入口：`/queue/{上游路径}` 形态、鉴权下沉上游、
   零资金动作、上游寻址、终态收敛与已知限制。当前代码就是它的实现。
2. **ADR-008** —— 与 stask 的分工：两服务按「包哪种上游 + 占哪个 URL 前缀」
   区分（`/async` vs `/async`），为什么不合并。跳过它会反复把 atask 与 stask
   搞混。
...
需要动手改 `app/services/relayflow.py` 或 `app/deps/ratelimit.py` 时，
**再加读 ADR-011**：它规定了并发槽的占用/释放时点与「恰好一次」的两种抢权
（放行权、还槽权），改错任一处都不报错，只表现为上游被多调一次或并发额度悄悄变多。

需要动**回调**（`app/services/notify.py`、`app/services/callback_addr.py`、受理段的
回调地址取值与摘除）时加读 **ADR-012**：它规定了地址从哪来（头优先、body 兜底）、
白名单为什么必须 fail-closed、为什么要把 body 里的地址从转发体摘除、以及为什么
**只推终态**。改错任一处不会报错，只表现为客户端收不到通知、或收到两份通知。
