...
客户端打 `/async/v1/tasks/{id}`（nginx 重写为网关内部的 `/queue/v1/tasks/{id}`）时期望拿到
**上游原生报文**（字段、顺序、未知字段、数值精度
...
以下七条是**必须写进运维文档**的登记项，不是待办清单。第 1–4 条取自本仓库 ADR-010
「已知限制」；第 5–7 条来自本仓库 ADR-012（回调地址准入与投递，它同时修订了 ADR-010
已知限制 5 的结论）：
...
4. **取消语义退化**：`DELETE` 只做尽力源头止损 + 本地置 `CANCELED`。上游取消形态
   ...
   这个形态，需改代码。
5. **转发体的「逐字节原样」有一个例外**：body 顶层带 `callback_url` 且处于默认「网关接管」
   模式时，该字段被**摘除**后再转发（转发体走 `data.submit_body`，`data.request_body` 仍存
   原文）。这反而**消除了双投递**——原样转发的话上游可能也回调一次。摘除只在该键存在时发生
   （头传地址时一个字节不碰 body）；`CALLBACK_PASSTHROUGH_UPSTREAM=true` 时无此例外
   （本仓库 ADR-012）。
6. **取消不投递回调**：`DELETE` 由客户端自己发起、结果它已知。取消路径刻意不复用
   `_finalize_queue`（它多一步「尽力源头止损」，且必须发生在清令牌会话**之前**——要用会话里
   的 sk 去发那个 `DELETE`），故**终态收口动作没有单一来源**：新增终态动作时必须同时问一句
   「取消路径要不要」。
7. **回调地址不防 DNS rebinding**：白名单配域名时，若该域名被解析到内网则发现不了。不做解析
   后校验是为了避免 TOCTOU（校验时的解析结果与 HTTP 客户端出站时的解析结果不保证一致）。
   信任边界交给 `CALLBACK_ALLOWLIST`（**空白名单 = 全拒**）。
