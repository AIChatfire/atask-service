...
> 实测起点为 100 测试，[3]–[12]、[17]、[19] 并不在代码里。**"138 测试"的出处
> 已查明（2026-08-17）**：`stash@{0}`（"temp before revert"，08-14 18:36）里是
> 旧 adapter 代（`app/adapters/{kling,seedance}` + `callbacks/{dispatcher,receiver}`
> + `billing/{outbox,renewer}`，`tests/test_w1_*`–`test_w6_*` 全套）——表头那句
> 描述的是那套已被 [14] 淘汰的架构，不是本树。本轮已按实际代码逐波补齐并回归
> （144 测试全绿）：[3][4]（W2）、[5][8]（W3）、[9][10]（W5）、
