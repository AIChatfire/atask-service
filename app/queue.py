...
- 延迟任务：ListRedisScheduleSource（`gw:sched:*`），由 scheduler 进程到期派发。
...
async def billing_settle_task(request_id: str, actual_amount: float,
    ...
    try:
        ...
        await _retry_or_dlq("BILLING_SETTLE", billing_settle_task.kicker(), context,
                            (request_id, actual_amount))


@broker.task
async def billing_cancel_task(request_id: str, context: Context = TaskiqDepends()) -> None:
    from app.services import taskstore
    from app.services.providers import billing
    try:
        await billing.cancel(request_id)
        await taskstore.mark_settled(request_id, 0)
    except Exception:
        log.exception("billing_cancel failed: %s", request_id)
        await _retry_or_dlq("BILLING_CANCEL", billing_cancel_task.kicker(), context, (request_id,))
...
async def publish_settle(request_id: str, actual_amount: float) -> None:
    await billing_settle_task.kiq(request_id, actual_amount)


async def publish_cancel(request_id: str) -> None:
    await billing_cancel_task.kiq(request_id)
...
async def schedule_poll(task_id: str, delay: int) -> None:
