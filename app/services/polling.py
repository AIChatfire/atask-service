...
async def poll_one(task_id: str) -> None:
    ...
    if not route or not route.probe_path or not upstream_task_id:
        return
    ...
    try:
        key = await providers.keys.lease(           # 按原 channel 钉回；keypool 不支持则忽略 key_id
            route.biz, model=data.get("model", ""),
            key_id=data.get("key_id"), group=data.get("key_group", "default"),
        )
    ...
    if mapped is None:
        ...
        await flow.finalize_task(task, mapped, resp)
